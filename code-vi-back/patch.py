...
async function benchmark() {
    ...
    const results = {
        ...
        save: { relational: 0, serverMemoryBytes: 0 },
    ...
    results.save.serverMemoryBytes = relSave.serverMemoryBytes;
    ...
    if (relSave.result) {
        ...
        console.log(`\n${'═'.repeat(60)}`);
        console.log('   📊 Query Benchmark Results — Server Memory Usage (MB)');
        console.log(`${'═'.repeat(60)}`);
        const toMB = (bytes) => (bytes / 1024 / 1024).toFixed(2);
        console.table({
            'Avg': { 'Whole JSON': toMB(stats.wholeJson.memory.avg), 'Natural Join': toMB(stats.join.memory.avg), 'Nested SQL': toMB(stats.nested.memory.avg) },
            'Median': { 'Whole JSON': toMB(stats.wholeJson.memory.median), 'Natural Join': toMB(stats.join.memory.median), 'Nested SQL': toMB(stats.nested.memory.median) },
            'Std Dev': { 'Whole JSON': toMB(stats.wholeJson.memory.std), 'Natural Join': toMB(stats.join.memory.std), 'Nested SQL': toMB(stats.nested.memory.std) },
            'P95': { 'Whole JSON': toMB(stats.wholeJson.memory.p95), 'Natural Join': toMB(stats.join.memory.p95), 'Nested SQL': toMB(stats.nested.memory.p95) },
        });
