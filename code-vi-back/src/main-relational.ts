...
import { AppRelationalModule } from './ast-relational/app-relational.module';
...
async function bootstrap() {
  ...
  const app = await NestFactory.create<NestExpressApplication>(AppRelationalModule, {
    httpsOptions,
  });
  ...
  const config = new DocumentBuilder()
    .setTitle('CodeVi API - Relational AST')
    .setDescription('AST 데이터를 정규화(Directory→File→Class→Function)하여 저장하는 버전')
    .setVersion('1.0')
    .build();
  ...
  const port = process.env.RELATIONAL_PORT || RELATIONAL_PORT;
  await app.listen(port, process.env.SERVER_HOST || '0.0.0.0');
  Logger.log(`🔗 [Relational AST Server] Running on port ${port}`);
